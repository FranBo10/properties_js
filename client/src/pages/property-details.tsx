import { Typography, Box, Stack, Divider } from '@pankod/refine-mui';
import { Person, Hotel, MeetingRoom, Bathtub } from '@mui/icons-material';
  ...
  return (
    <Box
      ...
      >
        ...
        fontWeight={700}
        ml={2} 
        color="#11142d">Détails</Typography>
      ...
      <Box mt="20px">
        <Typography fontSize={18} fontWeight={600} color="#11142d">Détails de la propriété</Typography>
        <Divider sx={{ borderColor: "#E4E4E4" }}/>
      </Box>

      <Stack mt="15px" direction="row" alignItems="center">
        <Person sx={{ color:"#808191" }}/>
        <Typography fontSize={16} fontWeight={500} color="#808191">Superficie: {propertyDetails.surface} m²</Typography>
      </Stack>

      <Stack mt="15px" direction="row" alignItems="center">
        <Hotel sx={{ color:"#808191" }}/>
        <Typography fontSize={16} fontWeight={500} color="#808191">Chambres: {propertyDetails.bedrooms}</Typography>
      </Stack>

      <Stack mt="15px" direction="row" alignItems="center">
        <MeetingRoom sx={{ color:"#808191" }}/>
        <Typography fontSize={16} fontWeight={500} color="#808191">Pièces: {propertyDetails.rooms}</Typography>
      </Stack>

      <Stack mt="15px" direction="row" alignItems="center">
        <Bathtub sx={{ color:"#808191" }}/>
        <Typography fontSize={16} fontWeight={500} color="#808191">Salles de bains: {propertyDetails.bathrooms}</Typography>
      </Stack>

      <Stack mt="15px" direction="row" alignItems="center">
        <Bathtub sx={{ color:"#808191" }}/>
        <Typography fontSize={16} fontWeight={500} color="#808191">WC: {propertyDetails.wc}</Typography>
      </Stack>

      <Box mt="20px">
        <Typography fontSize={18} fontWeight={600} color="#11142d">Equipements</Typography>
        <Divider sx={{ borderColor: "#E4E4E4" }}/>
      </Box>

      {/* <Stack mt="15px" direction="row" alignItems="center" flexWrap="wrap">
        {propertyDetails.equipments.map((equipment: string, index: number) => (
          <Box key={index} sx={{ bgcolor: '#f8f8f8', color: '#808191', px: 1, py: 0.5, borderRadius: '5px', mr: 1, mb: 1 }}>
            <Typography fontSize={14} fontWeight={400}>{equipment}</Typography>
          </Box>
        ))}
      </Stack> */}

        <Box mt="20px">
          <Typography fontSize={18} fontWeight={600} color="#11142d">Avis</Typography>
          <Divider sx={{ borderColor: "#E4E4E4" }}/>
        </Box>
                </Box>
              </Box>
